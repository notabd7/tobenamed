...
export default function StudyMaterialOverview() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex justify-center items-start pt-12">
      <div className="max-w-7xl w-full relative z-10"> {/* Increased max-width */}
        <div className="flex justify-start relative top-0 left-5 z-5">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              className={`px-6 py-3 border-2 border-gray-300 rounded-t-md mr-3 text-base cursor-pointer z-5 transition-all duration-300 ease-in-out ${
                activeTab === tab.id
        ...
        <div className="bg-white p-8 rounded-lg shadow-md z-10" style={{ minHeight: '800px' }}> {/* Increased padding and added minHeight */}
          <div className="text-xl"> {/* Increased base font size */}
